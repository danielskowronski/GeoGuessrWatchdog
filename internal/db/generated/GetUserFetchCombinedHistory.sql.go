...
// versions:
//   sqlc v1.31.1
// source: GetUserFetchCombinedHistory.sql
...
const getUserFetchCombinedHistory = `-- name: GetUserFetchCombinedHistory :many
SELECT
    fetch_id,
    fetch_timestamp,
...
func (q *Queries) GetUserFetchCombinedHistory(ctx context.Context, userID string) ([]GetUserFetchCombinedHistoryRow, error) {
	rows, err := q.db.Query(ctx, getUserFetchCombinedHistory, userID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []GetUserFetchCombinedHistoryRow
	for rows.Next() {
		var i GetUserFetchCombinedHistoryRow
		if err := rows.Scan(
			&i.FetchID,
			&i.FetchTimestamp,
			&i.DivisionName,
